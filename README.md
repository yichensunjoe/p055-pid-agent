...
用 TypeSafe System One 判读画图（面板里配置 API Key，代码出候选、模型只回答“指的是哪一个”、低置信度子句跳过而不是猜）见 [`docs/typesafe-drawing.md`](docs/typesafe-drawing.md)。完整产品边界见 [`docs/product-vision.md`](docs/product-vision.md)。Agent Harness 第一阶段的统一工具契约见 [`docs/tool-registry.md`](docs/tool-registry.md)，Session 与 Permission/Approval Gate 见 [`docs/agent-harness-sessions-permissions.md`](docs/agent-harness-sessions-permissions.md)，工程语义差异契约见 [`docs/semantic-diff.md`](docs/semantic-diff.md)。
...
`pid-agent repair-coverage` 跑 coverage ledger：`repair_planner` 认识、但当初冻结语料造不出来的 7 个 code，各自去哪了。**coverage promotion 已把它们结算完**：4 个能被这个仓库的文档持有的 code（`DUPLICATE_LABEL` / `PORT_DIRECTION_MISMATCH` / `UNBRIDGED_CROSSING` / `ANNOTATION_OVERLAP`）已晋升为冻结语料的正式 case（spec v3 / corpus v3，23 operator），ledger 逐条从冻结 generator 取出它那一条 case 并证明 code 真的被制造出来、真被 oracle 修好；另 3 个 code 进 disposition ledger，写面与导入面都拒绝持有该缺陷（因此不是“漏测”而是产品入口的纵深防御），一旦哪天变得可达，守卫测试立刻红并要求晋升。旧 spec v2 的 body 已归档，`spec_fingerprint("2")` 仍可复算。`pid-agent repair-scale [--source FILE.dwg]` 把同样的五个案例放到大图上跑，逐条报 scope size、context bytes/elements、attempts、各阶段耗时、RSS 与 wall-clock；`pid-agent repair` 在 CLI 上修一条 finding（先预览、再受治理写入）。真实模型资格用 `pid-agent repair-qualification`：没有凭据时它明确返回 `awaiting_real_model_qualification`（退出码 3），既不算通过也不算候选失败。
