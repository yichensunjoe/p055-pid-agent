...
- 结论做法：去掉无条件 `|| true`，让更新步骤本身成为真门禁；把渲染器从 `ubuntu-latest` 钉到 `ubuntu-24.04`（基线只对产出它的渲染器有意义）；用**与生成步骤绑定的 sentinel** 证明更新真的跑过——更新前**删除**一张已提交基线（另存备份），更新后要求它**被重新生成**、比 marker 新、并与备份**逐字节相同**（最后一条作为证据输出，不当作失败条件）。删除之所以比“篡改内容”好：**缺失的快照一定会被写回**，而换成尺寸不同的伪 PNG 时 Playwright 会因尺寸不符而**拒绝更新**，结果门禁在“本该正常”的场景下直接红。
