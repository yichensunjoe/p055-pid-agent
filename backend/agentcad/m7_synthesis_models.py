...
PROPOSAL_EVIDENCE_SCHEMA = "m7-proposal-evidence/1"
...
def derive_completeness(*, proposed: int, accepted: int, rejected: int) -> Completeness:
    ...
    if min(proposed, accepted, rejected) < 0:
        problems = "counts must not be negative"
...
class SynthesisProposalEvidence(StrictModel):
    ...
    # What was proposed, in full: this is the part that was previously unrecoverable.
    ...
    compiled_operation_count: int = Field(ge=0)
    rejected_operation_count: int = Field(ge=0)
    ...
    def problems(self) -> list[str]:
        ...
        if self.completeness == "partial" and self.rejected_operation_count <= 0:
        ...
        for receipt in self.rejected_operations:
            if not receipt.reason_code:
                problems.append(f"rejected operation {receipt.operation_id!r} has no reason_code")
...
def build_proposal_evidence(
    *,
    ...
    """Assemble one evidence row, deriving the axis that must not be supplied."""
