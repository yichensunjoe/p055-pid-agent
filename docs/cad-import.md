...
**许可边界（重要）**：AutoCAD / LibreDWG / ODA 都只作为**外部可执行工具**被 `subprocess`
调用，不链接、不复制、不 vendor；AutoCAD 更是只使用用户已经合法安装并授权的那一份，
程序本身不附带也不分发任何 Autodesk 二进制。FreeCAD 的 `importDWG.py` 之所以不能搬，是因为
它本身就是“`subprocess dwg2dxf` 的一层薄封装”（LGPL 代码），搬过来既没有多出能力，也会污染
本项目的版权归属。读 DXF、展开对象流、生成文档这三件事的代码全部在本仓库内，属自有实现。
**装了 AutoCAD 的机器上，DWG 导入全程不使用任何 GPL 组件**（DXFOUT 由 AutoCAD 完成，DXF 由
我们自己的读取器解析）。
...
`preserve_colors`、`max_elements`、`chunk_size`。
...
| 字段 | 含义 |
|---|---|
| `source` | 文件名、格式（`dxf`/`dwg`）、格式细节（`ASCII`/`AC1024`…）、字节数、**SHA-256**、编码、使用的转换器与完整命令行 |
...
| `operations` / `revisions` / `transactions` / `duration_ms` | 落地细节：多少操作、几个 revision、几个事务、耗时 |
...
1. `DocumentService` 创建**一个新文档**（`name` 默认 `<文件名> (CAD 导入)`）；
2. 图层与图元按 `chunk_size`（默认 1000/事务）分批提交，走 `POST /documents/{id}/transactions`
   的同一套代码路径，于是 revision 校验、历史、语义 diff、审计、undo 全部继承；
3. 文档 `metadata` 记录 `cad_source`（文件名、格式、SHA-256、转换器、帧、统计）；
4. **它不能修改、重新版本化或删除任何既有文档**——`surface_contract.py` 把这条写成
...
2. 它是**脚本驱动**的（`/s script.scr`），而 AutoCAD 命令行里**空行等于重复上一条命令**，
   ...
   守着这条）。它也不支持 `--version`，版本证据改从启动横幅读取。
...
1. **符号语义仍需人工映射**：287 个块实例只把块名写进 `dwg_block` 元数据；把它变成图例符号
   需要一张人工确认的映射表。
...
5. **大图仍需分批**：默认 1000 图元/事务，9242 图元 ≈ 9 s、10 个 revision；这是刻意的事务边界，
   不是性能上限。
