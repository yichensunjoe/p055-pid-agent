...
6. **单调（monotone）**：每个阶段只有在**不使任何硬性图面指标变差**时才被接受，否则整阶段
   ...
   比较的**参照点是“上一个已被接受的状态”**，不是整轮开始时的输入：pass 之间本来就互相
   干扰（挪一个设备会作废原先最优的走线），所以“最终比输入好”是不够的——那会允许后面的
   阶段把前面刚赢得的改善吐回去。上面的 `DRAFT_RESULT_REGRESSION`（最终结果 vs 输入）
   是**第二层保险**，不是全部保证。`backend/tests/test_drafting_engine.py` 里有两道回归：
   一道用 5 → 1 → 4 证明 B 阶段必须回滚，一道逐次检查比较参照点只能是最近接受态。
...
* 版本历史：**v1** 首个确定性整理流水线；**v2** 单调接受判据改为阶段局部（参照最近接受态）。
  版本提升意味着同一张图在 v1/v2 下的摘要**不保证相同**，这是刻意的——摘要必须能区分
  两种接受语义。
* 幂等性有测试：把引擎的输出再喂回引擎，`settled=True` 且 `transaction=None`。

## 10. 老图纸与“不可用元素”的降级契约

真实项目数据库里存在**引用了当前图例库已经不存在图例**的图纸（例：`pressure_transmitter`
不是图例 key，是历史生成器/旧版本留下的）。`DocumentService` 对这类元素会**正当地拒绝**
计算端口坐标，因此整理引擎必须能绕过它们，而不是在某个 pass 中途抛异常。

规则（`unresolvable_element_ids`）：

* 未定义图例的符号，**以及绑定到它的管线**，一起从所有 pass 的候选集中排除；
* 报告里给出一条 `DRAFT_SYMBOL_DEFINITION_MISSING` 警告（它**不**阻断门禁，因为“这张图
  是否合格”属于图面规则与 IR findings 的判断）；
* 锁定来源保持干净：这类元素**不会**被写成“被锁定”，否则锁定取证就在骗人；
* 即便仍有个别元素让某个阶段失败（例如服务端拒绝写入），该阶段也会被跳过、记
  `DRAFT_STAGE_UNAVAILABLE` 并停在**最后一次成功提交**上（每个阶段都在副本上工作，
  只有完成的阶段才提交），而不是让只读请求 500。

也就是说：**一张有历史缺陷的图纸仍然能拿到报告和可复现事务，只是报告会明说哪些部分
它动不了**。这条是真实数据发现的，不是设想：补丁前 24 张非空图纸里有 2 张让 `/drafting/preview`
返回 500。

## 11. 真实数据上的实测（本地活库，只读）

对本地活项目库中 **24 张非空图纸**（元素数 19 → 448）逐个跑 `POST /drafting/preview`：

| 指标 | 结果 |
|---|---|
| HTTP 200 | **24 / 24**（修复前 22 / 24 ，2 张 500） |
| 产出事务 | 17 张 |
| 已收敛（0 操作） | 7 张 |
| 分数提升 | 11 张 |
| 报告的回归 | **0**（单调门禁在真实数据上成立） |
| 写入 | 0（两个路由都是只读） |
| 耗时 | 中位数 ≈ 4.2 s；最慢 ≈ 45 s（448 图元，v1/v2 同量级） |
| 每张图两次运行摘要一致 | **24 / 24**（v1 与 v2 均成立） |

### 11.1 v1 → v2 的实测差异（同一活库快照，两个 git worktree 各跑一遍）

接受语义从“与输入比”改成“与最近接受态比”之后，同一张 448 图元图纸：
...
| 耗时 | 45.7 s | **40.4 s** |
| 最终回归（vs 输入） | 0 | 0 |
| 仍能拿到的改善 | crossings 7→0、text_connector 27→0、pipe_obstacle 89→64 … | crossings 7→0、text_connector 27→0、text_symbol 29→0、duplicate_label 25→17 |

差额 98 个操作就是旧实现放行过的“相对上一接受态恶化”的阶段。代价要说清楚：
**硬指标之间不再允许此消彼长**，所以冲突图纸上 v2 比 v1 保守（400+ 图元的这张图
`pipe_obstacle_intersections` 不再下降，因为它只能与 crossings 三选一）。这是单调契约要求
的语义，不是性能或质量退化；要让两者兼得，需要让后续 pass **部分接受**（只落地不冲突的
那部分改动），而不是整阶段二选一——这是后续任务，不是 M3 遗留 bug。

两点必须诚实写下：

* **大图性能是已知债务**。`commit()` 每接受一个阶段都要对整张图重新做一次图面质量分析，
  而流水线最多跑 3 轮 × 多个 pass，所以开销随图元数明显上升。448 图元 ≈ 48 s 属于“能用但
  不可接受”。应急手段是 `policy.pipeline_rounds=1`（或 `relayout=false` 只做局部整理）；
  正确修法是把硬性指标做成增量计算/缓存，属于后续任务，不应在下游功能里藏起来。
* **老图纸的“不可用元素”边界**（上节）与**声明式稳定标识缺失**（不同层职责）都会持续出现在
  真实数据里；它们被如实报告，但没有被“修好”——修它们需要上游数据质量或其它里程碑的能力。
...
* `backend/tests/test_drafting_geometry.py`：几何/端口/跨线/连接点/保留区/摘要、以及
  “未知图例 → 警告 + 不可用集合”的降级契约。
...
* `backend/tests/test_drafting_api.py` / `test_drafting_cli.py`：REST 与 CLI 面（含退出码），
  并使用真实数据里的同一形状（引用了未知图例的历史图纸）回归“两个只读路由不得 500”。
